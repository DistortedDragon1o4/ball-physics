...
void Physics::doPhysicsTick() {
	...
	if (sqrt(pow(velocity.x, 2) + pow(velocity.y, 2)) > 0.000006f)
		velocity = velocity - (glm::normalize(velocity) * 0.000006f);
	else
...
void Physics::generateNormals() {
	...
	for (int i = 0; i < size(points); i++) {
		if (i == 0)
			pointNormals.push_back(glm::normalize(normals.at(i) + normals.at(size(points) - 1)));
		else
...
void Physics::generateMesh() {
	float angle = PI / 180 * (360 / 90);
	for (int i = 0; i < 90; i++) {
		std::vector<float> part;
		float currentAngle = angle * i;
		part.push_back(sin(currentAngle) * 0.9);
