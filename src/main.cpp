...
int main() {
	...
	ball.Pos.y = 0.0f;
	ball.gravity.x = 0.0f;
	ball.gravity.y = -0.000098f * 2.0f;
	ball.velocity.x = -0.002f;
	ball.velocity.y = 0.0055f;
	...
	glBufferData(GL_ARRAY_BUFFER, size(ball.mesh) * sizeof(float), &ball.mesh[0], GL_STATIC_DRAW);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glBindVertexArray(envVAO);
		glDrawArrays(GL_LINE_LOOP, 0, size(ball.mesh) * 0.5f);
